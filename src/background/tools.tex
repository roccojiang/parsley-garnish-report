...
Scalafix rules can be either \emph{syntactic} or \emph{semantic} depending on whether they require semantic information to perform their analysis~\cite{scalacenter_scalafix_2024}.
Syntactic rules are faster to run, operating purely on the \textsc{ast} without requiring compilation to extract semantic information, but are more limited in the analyses they can perform.
Semantic rules are more complicated to run as they require compilation with the SemanticDB compiler plugin, but they enable more powerful analyses that require access to compiler information such as type signatures.
...
Ideally, users should be able to see inline diagnostics, as well as preview and select auto-fixes interactively.
...
\subsection{Guide to Scalafix}\label{sec:scalafix}
This \namecref{sec:scalafix} briefly explains how Scalafix rules are implemented, and key pieces of terminology in its \textsc{api}.
...
The simple syntactic rule in \cref{fig:syntactic-rule-ex} shows how this is achieved using the Scalafix \textsc{api}:
\begin{itemize}
...
A careful reader may notice from \cref{fig:syntactic-rule-ex} that the \scala{Patch} rewrite method receives a simple string value, which seems unsafe and could potentially lead to malformed code.
That careful reader would be correct.
