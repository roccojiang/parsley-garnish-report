...
\subsection{Normalisation Approaches}\label{sec:normalisation-approach}
Deciding on a concrete \textsc{adt} representation for the $\lambda$-calculus is a surprisingly complex problem.
This \namecref{sec:normalisation-approach} explores the different approaches to normalisation, and the effect this has on how the $\lambda$-calculus may be represented as a datatype.
We first explain the traditional notion of reduction-based normalisation, before introducing normalisation by evaluation as a more elegant alternative.
...
This rule is therefore a \emph{directed} notion of reduction, which can be implemented as a term-rewriting system, in a similar way to how \scala{Parser} terms are simplified.
...
The example below shows how the simply-typed $\lambda$-calculus can be represented as a generalised algebraic data type (\textsc{gadt}) in Scala:
...
\emph{Normalisation by Evaluation} (\textsc{nbe})~\cite{filinski_nbe_2004} achieves this by evaluating syntactical terms into a semantic model, then \emph{reifying} them back into the syntactic model.
