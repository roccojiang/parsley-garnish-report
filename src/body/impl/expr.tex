...
Currently, parsers such as \scala{pure} and \scala{map} take expressions as \scala{scala.meta.Term} values, which have been treated as black boxes.
...
\end{enumerate}
%
This helps achieve the following desired properties:
\begin{itemize}
...
\end{itemize}
This structure is largely mirrored in the \textsc{hoas}-based \scala{Sem} \textsc{adt} shown in \cref{fig:sem-adt}, which allows it to be reified back into \scala{Expr} terms.
...
\subsection{Lifting to the Intermediate Expression \textsc{ast}}
...
\TODO{TODO: highlight changes like Jamie's PhD thesis? Can I get the latex source for the nicely configured tcolorboxes lol}
\begin{minted}{scala}
...
As a high-level overview, there are three cases to consider during this conversion:
...
The body is also transformed to make sure references to these bound variables use their new names: this conversion is hygienic as it compares terms using their symbols, which Scalameta guarantees to be unique per source file.
...
The resulting \scala{Expr} term would then resemble the following $\lambda$-calculus expression:
\begin{lstlisting}
\(x1). \(x2, x3). Translucent(%\textbf{x2 + x3}%, env = {%\textbf{x1}% -> x1, %\textbf{x2}% -> x2, %\textbf{x3}% -> x3})
\end{lstlisting}
\textbf{Bold} values correspond to \scala{scala.meta.Term} nodes as opposed to \scala{Expr} values: therefore, the lambda body's environment maps \scala{Term.Name} nodes to their corresponding variable terms.
When this term is pretty-printed, the \scala{Term.Name} nodes are replaced with the corresponding \scala{Expr} terms -- this is analogous to the splicing operation on quasiquotes.
...
The interesting case is evaluating function application: the function and its arguments are evaluated separately at first.
The magic of \textsc{nbe} takes place here -- if the function evaluates to an abstraction, the arguments are passed to the Scala function \scala{g: List[Sem] => Sem}, collapsing the structure by one step.
%
\begin{minted}{scala}
trait Expr {
  def evaluate: Sem = {
...
Once the syntactic terms are fully evaluated into their semantics, the expression has been normalised to $\beta$-\textsc{nf}.
Reification is then rather simple, just converting each level of the term back into its syntactic counterpart.
When a lambda abstraction is reified, bound variables are assigned names from a fresh name supply.
This step is what grants $\alpha$-equivalence for free, as the fresh name generator can be made deterministic: given two terms of the same semantic structure, reifying both will yield syntactic representations with the same names.
%
\begin{minted}{scala}
trait Sem {
  def reify: Expr = {
...
\TODO{TODO: comparison with parsley haskell}
