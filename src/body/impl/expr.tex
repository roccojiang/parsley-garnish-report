...
This \namecref{sec:normalisation-approach} briefly explains the traditional notion of reduction-based normalisation, before introducing normalisation by evaluation as a more elegant and efficient alternative.
...
\begin{enumerate}
  \item Scalameta terms are lifted to the \scala{Function} \textsc{adt}, which represents the syntax of terms using a simple named approach.
  \item \scala{Sem} uses \textsc{hoas} to leverage Scala's semantics as the denotational model. During normalisation, \scala{Function} terms are evaluated into \scala{Sem}, then reified back into \scala{Function}.
\end{enumerate}
%
This helps achieve the following:
\begin{itemize}
  \item The syntactic \scala{Function} \textsc{adt} is represented in a simple manner, which is easy to construct and manipulate as opposed to a \textsc{hoas} representation. This allows function terms to be pattern matched on, as part of parser simplifications.
  ...
  \item Reifying \scala{Sem} terms back into syntactic \scala{Function} terms automatically $\alpha$-converts names, giving us $\alpha$-equivalence for free.
\end{itemize}
...
\begin{tikzcd}[row sep=3.75em,column sep=0.9em]
  ...
  \end{tikzcd}
\end{equation*}
\caption{Normalisation by evaluation for the \scala{Expr} datatype.}
\end{figure}
