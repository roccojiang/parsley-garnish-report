...
The unfolded \scala{result} portion can be reasoned with by the combinators' correspondence to \textsc{peg} expressions, as shown in \cref{tab:base-case-empties}. If the combinator can succeed by parsing the empty string, \scala{result} is a \scala{Some} value of its semantic action, otherwise it is \scala{None}.
