...
\section{Problem}
Left-recursive grammar rules are a common pattern to represent left-associativity.
...
It would therefore be ideal if \texttt{parsley-garnish} could complement this functionality with an auto-fix rule to refactor left-recursive parsers to use \texttt{parsley}'s idiomatic \scala{chain} combinators.
...
\begin{itemize}
  \item \scala{empty}: The semantic actions of the parser, if it can derive the empty string. Conceptually, this has type \scala{Option[A]} where \scala{A} is the type of the semantics.
...
\end{itemize}
%
This unfolded parser can then be recombined using the \scala{chain.postfix} combinator.
...
The following left-recursive parser and its transformation into a non-left-recursive form will be used as a representative example for the remainder of this \namecref{sec:factor-leftrec}:
\begin{minted}{scala}
lazy val example: Parsley[String] = (example, string("a")).zipped(_ + _) | string("b")
\end{minted}
%
The \scala{example} parser intends to express the following simple grammar expressed using left-recursion. The goal is to refactor \scala{example} so that it retains the intended semantics, but uses the \scala{postfix} combinator so that \texttt{parsley} can parse it correctly.
\begin{equation*}
