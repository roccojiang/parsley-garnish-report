...
\begin{itemize}
  ...
  \item The import path for the \emph{Implicit Lexer} pattern is subject to how the user names their lexer object, so determining this is much more heuristic-based.
  \begin{itemize}
    \item An import containing the keyword \scala{lexer} as well as the keyword \scala{implicit(s)}.
    \item 
    The presence of an import containing the keyword \scala{lexer} as well as the keyword \scala{implicit(s)}, or an importee called \scala{implicitSymbol}
  \end{itemize}

  if present, indicates the \emph{Implicit Conversions} pattern.
\end{itemize}
