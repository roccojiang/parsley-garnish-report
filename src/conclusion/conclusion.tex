...
Implementing the left-recursion factoring transformation in \texttt{parsley} itself would allow users to directly write parsers in left-recursive form without needing to transform it to use a \scala{chain} combinator, whether by hand or via \texttt{parsley-garnish}.
This is obviously possible in \texttt{parsley}: \cref{sec:simplify-parsers} discusses the relationship between the \texttt{parsley-garnish} \scala{Parser} \textsc{ast} and \texttt{parsley}'s own internal \textsc{ast}.
